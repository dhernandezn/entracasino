...
class Consultas
{
	...
	// echo $fecha;exit();
	$val = strtolower($val);
	$listar10 = $this -> revisarDobleIngreso($val);
	//print_r($listar10);
	//echo $listar10["condicion"];
	//echo "\t";exit();
	
		
		if($listar10["rut_ingre"] === $val && $listar10["fecha"] === $fecha){
			//print_r($rutLista);
			//echo $listar10["condicion"];
			return $this -> mensaje = "<input type='hidden' name='val666' value='5' id='val1'> Cliente Ya Registrado <br><br> 
			<input type='hidden' name='condicion' value='".$listar10['condicion']."' id='condicion'>
	...
	public function revisarDobleIngreso($rut){
		...
		$dbh = Database::getInstance();
		$listar10 = $dbh -> prepare("SELECT * from log WHERE rut_ingre = :rt ORDER BY id_log DESC LIMIT 1");
		$listar10 -> bindValue(":rt",$rut);
		$listar10 -> execute();
		$ultimos = $listar10->fetch(PDO::FETCH_ASSOC);
		//print_r($ultimos);
		//echo $ultimos['cli_prohibido'];exit();
		if($ultimos["cli_prohibido"]=='si'){
			$ultimos['condicion'] = 'CLIENTE PROHIBIDO';
		}
		if($ultimos["autoexc"]=='si'){
			$ultimos['condicion'] = 'CLIENTE AUTOEXCLUIDO';
		}
		if($ultimos["cli_pep"]=='si'){
			$ultimos['condicion'] = 'CLIENTE PEP';
		}
		if($ultimos["cli_sospechoso"]=='si'){
			$ultimos['condicion'] = 'CLIENTE SOSPECHOSO';
		}
		if($ultimos["cli_prohibido"]!='si' && $ultimos["autoexc"]!='si' && $ultimos["cli_pep"]!='si' && $ultimos["cli_sospechoso"]!='si'){
